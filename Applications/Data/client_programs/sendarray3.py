...
packet_payload_struct = struct.Struct('31s')
...
def main():
    ...
    fragment_data(frag_data_list, 300)

def fragment_data(fragment_data_list, frag_size):
    for i in range(0,(len(fragment_data_list)/frag_size)+1):
        if i == 0:
            index_bottom = 0
        else:
            index_bottom = i*frag_size-1
        if(((i+1)*frag_size)>len(fragment_data_list)):
            index_top = len(fragment_data_list)-1
        else:
            index_top = (i+1)*frag_size-1
        print index_bottom, index_top

        send(fragment_data_list[index_bottom:index_top])

def send(frag_data_list):
    sequence_cnt = 0
    temp_list = []
    ...
    for item in frag_data_list:
        if frag_data_list_len == 1:
            ...
            if(sequence_cnt<254):
                # Create datapacket
                data_tx = packet_msg_struct.pack(str(proxylocalcallsign).upper(), proxylocalnodeid, 0, len(item), str(item))
            else:
                # Create datapacket
                data_tx = packet_msg_struct.pack(str(proxylocalcallsign).upper(), proxylocalnodeid, 0, len(item), str(item))
