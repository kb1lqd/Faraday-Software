...
list_packets = []
# Frame Definitions
pkt_datagram_frame = struct.Struct('1B 31s')  # Fixed
pkt_start = struct.Struct('9s 3B')  # Fixed
pkt_data = struct.Struct('2B 29s')  # Variable  Data Length
pkt_end = struct.Struct('1B')  # Fixed
...
def fragmentcount(msg_len, MAX_MSG_DATA_LENGTH):
    """
    ...
    frag_cnt = msg_len / MAX_MSG_DATA_LENGTH
    if msg_len % MAX_MSG_DATA_LENGTH > 0:
        frag_cnt += 1
    return frag_cnt

def fragmentmsg(msg, MAX_MSG_DATA_LENGTH):
    """
    ...
    """
    list_message_fragments = [msg[i:i + MAX_MSG_DATA_LENGTH] for i in
                              range(0, len(msg), MAX_MSG_DATA_LENGTH)]
...
def createmsgpackets(src_call, src_id, msg):
    """
    ...
    # Create START Packet
    msg_start = createstartframe(src_call, src_id, len(msg))
    msg_start = pkt_datagram_frame.pack(MSG_START, msg_start)

    # Create END Packet
    msg_end = createendframe(len(msg))
    msg_end = pkt_datagram_frame.pack(MSG_END, msg_end)

    # Create DATA Packet(s)
    list_msg_fragments = fragmentmsg(msg, FRAGLEN)
    list_data_packets = []

    del list_data_packets[:]  # Remove all old indexes

    for i in range(0, len(list_msg_fragments), 1):
        data_packet = createdataframe(i, list_msg_fragments[i])
        # print "Pre-Pack:", repr(data_packet), len(data_packet)
        data_packet = pkt_datagram_frame.pack(MSG_DATA, data_packet)
    ...
    list_packets = []  # Reset any old packet fragments
    del list_packets[:]  # Remove all old indexes
    list_packets.append(msg_start)
    for i in range(0, len(list_data_packets), 1):
        list_packets.append(list_data_packets[i])
    list_packets.append(msg_end)

    return list_packets


def createstartframe(src_call, src_id, msg_len):
    """
    ...
    frag_cnt = fragmentcount(msg_len, FRAGLEN)
    # print frag_cnt
    # Create packet
    packet = pkt_start.pack(src_call, len(src_call), src_id, frag_cnt)
    # Return packet created
    return packet


def createdataframe(sequence, data):
    """
    ...
    packet = pkt_data.pack(sequence, len(data), data)
    # print "created:", repr(packet), len(packet)
    return packet


def createendframe(msg_len):
    """
...
class rxparse(object):
    ...
    def parseframe(self, rx_byte, startbyte, stopbyte, escapebyte,):
            # PARSE BYTES
            ...
            # Check PACKET STARTED Flag = True (Packet already started and being recieved)
                ...
                if not self.logic_escapebyte_received:
                    ...
                    if ((rx_byte != escapebyte) and (rx_byte != startbyte) and (
                        rx_byte != stopbyte)):
                        self.partial_packet += rx_byte
                    # Escape byte received
                    elif (rx_byte == escapebyte):
                ...
                # Check if current BYTE is being escaped (framing) - TRUE
                    ...
                    if ((rx_byte == escapebyte) or (rx_byte == startbyte) or (
                        rx_byte == stopbyte)):
                        self.logic_escapebyte_received = False
                        self.partial_packet += rx_byte
                    # Unknown State - Error
